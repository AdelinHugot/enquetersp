...
const ICON_MAP: Record<string, LucideIcon> = {
  Ban, Car, Waves, Snowflake, Thermometer, Sparkles, Plus,
};
...
const PriorityInput: React.FC<{
  ...
  onSubmit?: () => void;
}> = ({ options, value, onSelect, otherText, onOtherTextChange, onSubmit }) => {
  return (
...
function getGridClass(question: Question): string {
  ...
  if (question.layout === 'grid') {
    ...
    return `grid grid-cols-1 ${cols} gap-4 items-stretch`;
  }
...
export default function App() {
  ...
  const submitToWebhook = async (type: 'decouverte' | 'rs', currentAnswers: Answers, questions: Question[]) => {
    ...
    const responses: Record<string, { question: string; section: string; answer: string; other_text: string }> = {};
                    ...
                    <div className={`flex-grow ${getGridClass(currentQuestion)}`}>
                      {currentQuestion.type === 'priority' ? (
                        ...
                        currentQuestion.options?.map((option) => {
                          ...
                          return (
                            <div key={option.id} className="flex flex-col gap-3 h-full">
                              <button
                                onClick={() => handleOptionSelect(option.id)}
                                className={`
                                  group w-full h-full flex flex-col items-center justify-center gap-3 p-5 text-center transition-all duration-300 overflow-hidden rounded-2xl border-2
                                  ${isSelected
                                    ? 'border-[#FFBE00] bg-[#FFBE00]/10 shadow-[0_4px_20px_rgba(255,190,0,0.08)]'
                                    : 'border-white/5 bg-white/5 hover:border-white/20 hover:bg-white/10'}
                                `}
                              >
                                {option.icon && (() => { const Icon = ICON_MAP[option.icon!]; return Icon ? <Icon className={`w-6 h-6 flex-shrink-0 transition-colors ${isSelected ? 'text-[#FFBE00]' : 'text-slate-400'}`} /> : null; })()}
                                <span className={`font-bold text-sm transition-colors leading-tight ${isSelected ? 'text-white' : 'text-slate-300'}`}>
                                  {option.label}
                                </span>
                              </button>
